...
| Pull Request | `PR` | The unit of change review and merge: a branch proposed for landing on the default branch. The kit lands every change via a PR, never a direct push — except destroying history on the default branch, which no PR can express ([ADR-0004](adr/0004-destroying-history-on-the-default-branch.md)). Note: not a `PRD` — see above. | The [`pre-push` hook](../.githooks/pre-push) refuses a direct push to `main`; a PR is how the change lands. |
...
| Work slice | `—` | One step of an issue's plan under [R12](issue-workflow.md#r12--slice-and-prioritize): a unit of work scoped to a single domain — a test slice, a datastore slice, a docs slice — that passes the [quality gate](engineering-discipline.md#working-a-task-under-the-quality-gate) on its own. Slices are ordered by dependency and TDD, the test slice first. Note: the "domain" a slice is scoped to is an area of the *work*; it is not the kit's "domain-free", which means the kit names no *product* domain. | A change to product code has a test slice (done first) and an implementation slice, each its own gate-passing step. |
| Forge | `—` | The hosted service holding the repository, its issues, and its pull requests. The kit is forge-free: it names no specific one, and forge-specific templates ship inert under [`templates/`](templates/). Note: most hosted forges keep a per-pull-request reference the operator cannot delete, so deleting a commit from a branch does not always remove it from the forge. | Forge-specific issue and pull-request templates ship inert under [`templates/`](templates/); copy them into place only when you adopt that forge. |
| Backup branch | `—` | A branch pushed to the remote holding a tip that condition 3 of [ADR-0004](adr/0004-destroying-history-on-the-default-branch.md) requires preserved — the default branch's tip and every other remote branch tip that is not an ancestor of it — named `backup/pre-<reason>-<short-sha>`, one per tip. Collision to watch for: not a backup *copy* of the repository — it preserves one commit inside the same repository. | Before a reset, the default branch's tip and every other branch tip on the remote that is not an ancestor of it each get one. |
| Operator | `—` | Anyone the kit's rules bind: each human and each LLM coding agent alike, as [`issue-workflow.md`](issue-workflow.md) states. Note: one rule narrows it — [ADR-0004](adr/0004-destroying-history-on-the-default-branch.md) requires the second operator approving a destruction of history to be a person, because an agent instantiated by the acting operator is not an independent party. | R4 needs two different operators to approve a workaround; either may be an agent. |
...
| Destruction of history | `—` | Making commits unreachable from the remote's default branch, where they were reachable from whichever branch was the default at any point in the project's history. [ADR-0004](adr/0004-destroying-history-on-the-default-branch.md) states which acts count, which sequences count, and the exceptions, and permits it only under five conditions. Note: rewriting local history you have never pushed is not this. | A reset of the published default branch to an earlier commit destroys history; rebasing an unpushed local branch does not. |
