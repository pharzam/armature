...
| Destruction of history | `—` | Making commits unreachable from the remote's default branch, and from every other branch on the remote, where they were reachable from whichever branch was the default at any point in the project's history: a reset, a force-pushed rewrite, or deleting the branch. Permitted only under the five conditions in [ADR-0004](adr/0004-destroying-history-on-the-default-branch.md). Note: rewriting local history you have never pushed is not this, and neither is pointing the default at another branch while the old one stays. | A reset of the published default branch to an earlier commit destroys history; rebasing an unpushed local branch does not. |
