...
| Destruction of history | `—` | Making commits unreachable from the branch the project treats as canonical, where they were reachable from whichever branch held that role at any point since: a reset, a force-pushed rewrite, or deleting the branch. Permitted only under the five conditions in [ADR-0004](adr/0004-destroying-history-on-the-default-branch.md). Note: rewriting local history you have never pushed is not this. | A reset of the published default branch to an earlier commit destroys history; rebasing an unpushed local branch does not. |
