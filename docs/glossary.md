...
| Issue-first workflow | `—` | The rule that an open issue precedes any change, and a PR links back to it — defined in [`issue-workflow.md`](issue-workflow.md) as R1–R12. A destruction of history has the issue but no PR (ADR-0004). | No commit starts without an open issue; the PR body says `Closes #N`. |
