...
| Backup branch | `—` | A branch pushed to the remote to hold the exact tip of the default branch before a [reset](adr/0004-reset-the-default-branch.md) discards it, named `backup/pre-<reason>-<short-sha>`. It is reference only, never a merge source: discarded work returns to `main` by being re-landed slice by slice, not by being merged back. Collision to watch for: not a backup *copy* of the repository — it preserves one commit inside the same repository, and it is deleted only once the work it holds has re-landed. | `backup/pre-r12-reset-999765f` holds the 45 commits the 2026-08-27 reset discarded. |
