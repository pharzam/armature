...
| Pull Request | `PR` | The unit of change review and merge: a branch proposed for landing on the default branch. The kit lands every change via a PR, never a direct push — except destroying history on the default branch, which no PR can express ([ADR-0004](adr/0004-destroying-history-on-the-default-branch.md)). Collision to watch for: not a `PRD` — see above. | The [`pre-push` hook](../.githooks/pre-push) refuses a direct push to `main`; a PR is how the change lands. |
...
| Forge | `—` | The hosted service holding the repository, its issues, and its pull requests. The kit is forge-free: it names no specific one, and forge-specific templates ship inert under [`templates/`](templates/). Note: most hosted forges keep a per-pull-request reference the operator cannot delete, so deleting a commit from a branch does not always remove it from the forge. | Whether a discarded commit stays fetchable after a history rewrite depends on what your forge retains. |
| Backup branch | `—` | A branch pushed to the remote holding a tip that a [destruction of history](adr/0004-destroying-history-on-the-default-branch.md) is about to make unreachable, named `backup/pre-<reason>-<short-sha>`, one per discarded tip, swept as the refs stood when the issue was opened *and* at the act. Collision to watch for: not a backup *copy* of the repository — it preserves one commit inside the same repository. Note: it does not exempt the act — a backup made because of a destruction is not what makes it not one. | Before a reset, the default branch's tip and every other branch tip on the remote that is not an ancestor of it each get one. |
...
| Destruction of history | `—` | Making commits unreachable from the remote's default branch, where they were reachable from whichever branch was the default at any point in the project's history: a reset, a force-pushed rewrite, or deleting the branch. Permitted only under the five conditions in [ADR-0004](adr/0004-destroying-history-on-the-default-branch.md). Note: rewriting local history you have never pushed is not this, and neither is pointing the default at another branch while the old one stays — that branch stood there independently of the act, where a backup pushed under condition 3 does not, or obeying the rule would put you outside it. | A reset of the published default branch to an earlier commit destroys history; rebasing an unpushed local branch does not. |
