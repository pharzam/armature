...
| Forge | `—` | The hosted service holding the repository, its issues, and its pull requests. The kit is forge-free: it names no specific one, and forge-specific templates ship inert under [`templates/`](templates/). Note: most hosted forges keep a per-pull-request reference the operator cannot delete, so deleting a commit from a branch does not always remove it from the forge. | An adopter on a self-hosted forge fills the `‹forge›` placeholders and copies the matching templates into place. |
| Backup branch | `—` | A branch pushed to the remote holding a tip that condition 3 of [ADR-0004](adr/0004-destroying-history-on-the-default-branch.md) requires preserved, named `backup/pre-<reason>-<short-sha>`, one per tip; condition 3 states the set, the two moments it is read at, and how long the references are held. Collision to watch for: not a backup *copy* of the repository — it preserves one commit inside the same repository. | A reset that discards four unmerged tips creates four of them, and none may be deleted while it still holds a commit the default branch has lost. |
...
| Destruction of history | `—` | Making commits unreachable from every branch that has been the default at any point in the project's history, where they were reachable from one of them. [ADR-0004](adr/0004-destroying-history-on-the-default-branch.md) states which acts count and permits it only under five conditions. Note: rewriting local history you have never pushed is not this. | A reset of the published default branch to an earlier commit destroys history; rebasing an unpushed local branch does not. |
