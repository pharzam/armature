...
| Portable Operating System Interface | `POSIX` | The standard that defines a common shell and utility set across Unix-like systems. In this kit it is why the [discipline tests](engineering-discipline.md#testing) are plain shell: a POSIX script runs anywhere without a toolchain, so a project has tests before it has a build. | [`adr-lint.sh`](adr/adr-lint.sh) uses only POSIX shell and awk, so it runs on a developer's machine and in a bare CI image alike. |
