...
| Backup branch | `—` | A branch pushed to the remote holding a tip that a [destruction of history](adr/0004-destroying-history-on-the-default-branch.md) is about to make unreachable, named `backup/pre-<reason>-<short-sha>`, one per discarded tip. Collision to watch for: not a backup *copy* of the repository — it preserves one commit inside the same repository. | Before a reset, the default branch's tip and every other branch tip on the remote that is not an ancestor of it each get one. |
