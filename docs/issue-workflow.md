...
| ------- | ------------ | ---------- | -- | ----------------- | ------ |
| Land only via a PR (never a direct push to the default branch), except the row below | R1 | [`pre-push`](../.githooks/pre-push) — the branch it is configured to protect; advisory, inert on a fresh clone | — | ‹require a PR before merge› | Hook ships; branch protection is your step |
| Destroy history on the default branch | [ADR-0004](adr/0004-destroying-history-on-the-default-branch.md) | [`pre-push`](../.githooks/pre-push) — the push path only; advisory, inert on a fresh clone | — | ‹block force-push and deletion on the default branch› — reaches every client, but only the acts, and a bypass holder passes; silent on who approved. It does not see the default pointer being moved | Written rule only — nothing checks the five conditions |
