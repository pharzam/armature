...
| `good-path with space` | `prd-lint: OK`, exit 0 | a **space in the case directory's own name**. The file list was a space-joined string looped over unquoted, so awk was handed two half-paths and could open neither ([#76](https://github.com/pharzam/armature/issues/76)). The kit ships no `PRD-*.md`, so this linter was green on its own tree and red for an adopter on the first real PRD — this case is what makes it visible here |
