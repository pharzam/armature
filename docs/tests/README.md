...
| [`discipline-tests.sh`](discipline-tests.sh) | The runner for the discipline linters' own fixtures. Three guarantees, and it needs all three: **why** — each `bad-*` case must provoke the message named in its sibling `<name>.expect`, because exit status alone cannot tell "caught the defect" from "could not open the file"; **how many** — each set's case count is pinned, so a case that disappears by any mechanism is a failure; **nothing skipped by accident** — a missing linter, a stray entry, or an orphaned `.expect` all fail, and deliberate skipping goes through a validated `SKIP_SETS`. |
| [`runner-selftest.sh`](runner-selftest.sh) | The runner's own tests: twelve scenarios, each mutating a copy of the tree in a way that once produced a pass the runner had not earned — a gutted fixture, a deleted linter, a missing fixture root, a fixture renamed to a dotfile, one replaced by a broken symlink, one deleted outright, every `good*` fixture removed, an empty `bad-*` directory with a crafted expectation, a blank line in an `.expect`, and `SKIP_SETS` given a glob. Each scenario asserts **which** message the runner produced, not merely that it failed. Runs in CI, not in the hook: about ten seconds. |
