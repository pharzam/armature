...
to a linter this record cites. A number stated as a measurement at a named commit,
rather than as a pointer, is left where it is; it is true of that commit.
...
| A1 | A trailing slash on the `adr-lint.sh` directory argument silences the cross-link check. `adr-lint.sh:134` filters on `^$adr_dir/`, which becomes a double slash and matches no path, so the ADR's own index README counts as an inbound link. The failure is a false negative: in any run that passes the index check at `:221`, the warning cannot fire. **True as audited, and no longer true.** `T-8q3f` separated the two checks: `:221` still matches a bare filename, but the cross-link check now needs a **link**, so a run can pass the index check and still warn. The rest of this row is unaffected — a trailing slash still stops the case directory being excluded, and a case whose index row *links* its record still silences the warning. `sh docs/adr/adr-lint.sh docs/adr/tests/good/` prints no `WARN`; the same run without the slash prints two. | Stands | low |
