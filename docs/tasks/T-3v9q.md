...
| -- | ------- | ------- | -------- |
| M1 | The duplicate-ADR-number check is `adr-lint.sh:317`. No case under `docs/adr/tests/` holds two records with the same number — every case directory has one file per number. Gut that check and the suite still passed at `b684a96`, where it printed `34 passed, 0 failed`. | Stands | medium |
| M2 | The ADR title-line check is `adr-lint.sh:340-342`, not one line. It has no fixture: every record under `docs/adr/tests/` starts with a correct `# NNNN. <title>` line. | Corrected | low |
| M3 | The `Date:` line check has no fixture. Its two `err` sites are `adr-lint.sh:348` (the line is missing) and `:356` (the format is wrong). All eight records under `docs/adr/tests/` carry the placeholder `Date: YYYY-MM-DD`, so neither site can fire. | Stands | low |
| M4 | The missing `## Status` check is `adr-lint.sh:361-362`. It has no fixture: every record under `docs/adr/tests/` has the section. `bad-status` tests the status value at `adr-lint.sh:368`, not the missing section. | Stands | medium |
...
| A1 | A trailing slash on the `adr-lint.sh` directory argument silences the cross-link check. `adr-lint.sh:249` excludes the ADR directory by its own path, which becomes a double slash and matches nothing, so the ADR's own index README counts as an inbound link. The failure is a false negative: in any run that passes the index check at `:378`, the warning cannot fire. **True as audited, and no longer true.** `T-8q3f` separated the two checks: `:378` still matches a bare filename, but the cross-link check now needs a **link**, so a run can pass the index check and still warn. The headline stands: a trailing slash still stops the case directory being excluded, and a directory whose index README *links* its record still silences the warning that way. The *mechanism* moved with it, twice: the `grep` this row cites is gone, replaced by a literal prefix comparison, because an operator path must not reach a pattern. The substance is untouched — the value compared still ends in `//` when the argument does, so it prefixes nothing and excludes nothing. The repro moved, though — the same change also made this check skip fixture case directories, so for a `good*/` or `bad-*/` case the slash no longer decides anything: `sh docs/adr/adr-lint.sh docs/adr/tests/good/` and the same run without the slash now both print two. A directory whose name is neither `good*` nor `bad*` still shows it: copy a case to `docs/adr/tests/zzz-notafixture/` and the run with the slash prints no warning while the run without prints one per record it holds. | Stands | low |
...
| A3 | The README row status is never compared to the record. `adr-lint.sh:378` is a bare filename match and `$readme` is read nowhere else, so the `Status` cells at `docs/adr/README.md:96-102` can say anything. `adr-lint.sh:367` also accepts `Superseded by ADR-0007` when ADR-0007 does not exist. | Stands | low |
...
| `adr-lint.sh:356` | the `Date:` **format** — separate from `:348`, which B counts as one check with it |
| `adr-lint.sh:373` | the `## Context` and `## Consequences` arms of the required-section loop; only `## Decision` has a fixture |
