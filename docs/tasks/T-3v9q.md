...
| -- | ------- | ------- | -------- |
| M1 | The duplicate-ADR-number check is `adr-lint.sh:219`. No case under `docs/adr/tests/` holds two records with the same number — every case directory has one file per number. Gut that check and the suite still passed at `b684a96`, where it printed `34 passed, 0 failed`. | Stands | medium |
| M2 | The ADR title-line check is `adr-lint.sh:237-239`, not one line. It has no fixture: every record under `docs/adr/tests/` starts with a correct `# NNNN. <title>` line. | Corrected | low |
| M3 | The `Date:` line check has no fixture. Its two `err` sites are `adr-lint.sh:245` (the line is missing) and `:253` (the format is wrong). All eight records under `docs/adr/tests/` carry the placeholder `Date: YYYY-MM-DD`, so neither site can fire. | Stands | low |
| M4 | The missing `## Status` check is `adr-lint.sh:258-259`. It has no fixture: every record under `docs/adr/tests/` has the section. `bad-status` tests the status value at `adr-lint.sh:265`, not the missing section. | Stands | medium |
...
| A1 | A trailing slash on the `adr-lint.sh` directory argument silences the cross-link check. `adr-lint.sh:180` filters on `^$adr_dir/`, which becomes a double slash and matches no path, so the ADR's own index README counts as an inbound link. The failure is a false negative: in any run that passes the index check at `:275`, the warning cannot fire. **True as audited, and no longer true.** `T-8q3f` separated the two checks: `:275` still matches a bare filename, but the cross-link check now needs a **link**, so a run can pass the index check and still warn. The headline stands: a trailing slash still stops the case directory being excluded, and a directory whose index README *links* its record still silences the warning that way. The repro moved, though — the same change also made this check skip fixture case directories, so for a `good*/` or `bad-*/` case the slash no longer decides anything: `sh docs/adr/adr-lint.sh docs/adr/tests/good/` and the same run without the slash now both print two. A directory whose name is not a fixture pattern still shows it. | Stands | low |
...
| A3 | The README row status is never compared to the record. `adr-lint.sh:275` is a bare filename match and `$readme` is read nowhere else, so the `Status` cells at `docs/adr/README.md:89-95` can say anything. `adr-lint.sh:264` also accepts `Superseded by ADR-0007` when ADR-0007 does not exist. | Stands | low |
...
| `adr-lint.sh:253` | the `Date:` **format** — separate from `:245`, which B counts as one check with it |
| `adr-lint.sh:270` | the `## Context` and `## Consequences` arms of the required-section loop; only `## Decision` has a fixture |
