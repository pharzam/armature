...
- **2026-08-28** — **T-7d2x** — Added ADR-0004: history on the default branch may be destroyed only under five conditions, one of which is a second operator who is a person, and the forge's surviving pull-request references are named — so rotation ends a credential exposure where deletion may not ([#43](https://github.com/pharzam/armature/issues/43); [detail](t-7d2x.md))
