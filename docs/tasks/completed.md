...
- **2026-08-27** — **T-7d2x** — Added ADR-0004: `main` may be reset only under a recorded procedure — a stated goal with the cheaper alternatives weighed, a backup branch on the remote, every issue whose deliverable the reset removes returned to open, and a record on the parent issue, all written before the reset rather than after; the "What is enforced where" table gains an honest row saying nothing backs it ([#43](https://github.com/pharzam/armature/issues/43))
