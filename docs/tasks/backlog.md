...
- **T-6f3w** — Fix `adr-lint`: check the index row's status against the record it names (the directory-argument half was closed by `T-8q3f`, which canonicalised the argument rather than stripping a slash — the slash was one of four spellings of one defect) ([#45](https://github.com/pharzam/armature/issues/45); [detail](T-3v9q.md))
