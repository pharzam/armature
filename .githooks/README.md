...
| Hook | Runs | Adapt? |
|------|------|--------|
| [`pre-commit`](pre-commit) | The ADR, PRD, glossary, and backlog linters, then the linters' own fixture tests, then your `‹lint›` / test levels (fast subset) / security checks (fast subset). | Fill the `‹…›` steps for your stack. |
