...
| Hook | Runs | Adapt? |
|------|------|--------|
| [`pre-commit`](pre-commit) | A provenance check on where the hooks came from, then the ADR, PRD, agent-entry and link linters and the discipline self-tests, then your `‹lint›` / test levels (fast subset) / security checks (fast subset). | Fill the `‹…›` steps for your stack. |
