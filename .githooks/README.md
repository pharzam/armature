...
| Hook | Runs | Adapt? |
|------|------|--------|
| [`pre-commit`](pre-commit) | The ADR linter and the PRD linter, then your `‹lint›` / test levels (fast subset) / security checks (fast subset). | Fill the `‹…›` steps for your stack. |
...
until `core.hooksPath` is set. The real lock lives server-side and, unlike this
hook, reaches every client — though a holder of a bypass can still pass it. It is
your host's
